...
namespace file{
    ...
    void virtualinode::appendBlcok(int idx) {
        ...
        //位于二级索引的范围内
            ...
            auto sp  = file_system.lock();
            if(block_num == DIRECT_CNT + PTRS_PER_BLOCK) double_indirect = sp->allocateOneBlock();//分配一个二级索引块
            if(double_indirect == -1) return;
            if(cut_double_indirect_idx == 0){//分配一个一级索引块
    ...
    int virtualinode::deleteData(int pos, int len) {//删除文件中的内容，没写释放空的文件块，可以补️以下
    ...
    std::vector<int> virtualinode::release(){//释放不用的文件块，以向量的形式返回所有释放的文件块的ID
        auto sp = file_system.lock();
        int lbn = 0;
        std::vector<int> ans;
    ...
    std::optional<std::vector<int>> virtualinode::deleteInode(std::string delete_name) {//删除节点时会将释放的文件块以向量的形式返回
        if(ref_count > 1 || name != delete_name) return std::nullopt;//要求名字为该文件的名字且无其他引用
        deleteData(0,size);
        auto sp = file_system.lock();
