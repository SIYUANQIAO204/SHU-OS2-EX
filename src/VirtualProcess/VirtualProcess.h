...
//
/*
 * 本文件中的函数用于模拟进程使用的过程，包括生成内存访问序列以及对资源的请求
 * 内存访问序列生成给定进程所需的（不是分配的）内存页数和序列长度，按实验要求中给出的局部性原理的实例生成访问序列
 * 资源请求生成函数需要得到所有进程的pid（来得到某个进程的下标），已分配的资源矩阵，最大资源需求矩阵，并返回pid以及对应的资源请求
 * */
