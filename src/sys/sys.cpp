...
namespace sys{
    ...
    bool sys::checkAndFinish(int CPU) {//检查进程是否完成
        if (CPU < 0 || CPU >= (int)ProcessManager.size()) return false;
        if (!ProcessManager[CPU]) return false;
        if(maxNeedCheck(CPU)){
    ...
    void sys::logout(int uid) {//用户登出，同时删除fdtable
        FileManager->deleteFDTable(uid);
    }

    std::shared_ptr<file::fdtable> sys::getFDTable(int uid) {//获取某一用户的fd表
        return FileManager->switchFDTable(uid);
    }

    void sys::init() {//初始化系统，创建必要的用户和根目录
